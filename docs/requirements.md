...
Validation extracts only `amount` and `currency`. Extra JSON properties are ignored, and the canonical fingerprint is computed from this validated payment rather than the raw JSON body. The same key conflicts only when the validated amount or currency differs.
...
Reusing a key with a different validated-payment fingerprint must return `409`, `X-Cache-Hit: false`, and:
...
A failed operation removes its `PROCESSING` claim rather than creating another state. `PROCESSING` records do not expire. A `COMPLETED` record expires exactly 24 hours after completion. A lookup for that key, including a claim lookup, removes an expired record and treats it as absent.

The repository exposes `deleteExpired()` for bulk cleanup, but the application does not schedule or invoke it. An expired record that is never looked up may remain physically in memory until a future cleanup hook or process restart.
...
- immediate conflicts for different validated payments, both in flight and completed;
